...
- **improved_longer**：**9** 个 `Conv2d`，与 exp6 相同下采样节奏但在 24×24 与 12×12 多叠卷积；**无残差**。
...
| run | 要点 |
|-----|------|
| **exp6_improved_long** | 增强 + `improved_long` + Adam `1e-3` + 头 Dropout 0.3 |
| **exp7_improved_longer** | 与 exp6 **相同训练配方**，模型换为 **`improved_longer`（9 层卷积）** |
...
| run | 模型 | 测试准确率 (%) | 备注 |
|-----|------|----------------|------|
| exp6_improved_long | improved_long | 80.40 | 验证峰值最高 82.71 |
| **exp7_improved_longer** | improved_longer | **81.60** | **当前测试最佳** |
